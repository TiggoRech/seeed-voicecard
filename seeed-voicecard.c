...
/*
 * SEEED voice card
 * Adaptado para compatibilidade com kernel 6.12+
 */
...
#include "simple_card_utils_local.h"
...
static int seeed_voice_card_startup(struct snd_pcm_substream *substream)
{
    ...
    struct seeed_dai_props *dai_props = seeed_priv_to_props(priv, rtd->num);
    int ret;
...
// Variável interna, não usada fora deste arquivo
static int (*_set_clock[_SET_CLOCK_CNT])(int, struct snd_pcm_substream *, int, struct snd_soc_dai *);

// Protótipo declarado antes da definição
static int seeed_voice_card_register_set_clock(int stream, int (*set_clock)(int, struct snd_pcm_substream *, int, struct snd_soc_dai *));
...
//		if (_set_clock[SNDRV_PCM_STREAM_PLAYBACK])
//			_set_clock[SNDRV_PCM_STREAM_PLAYBACK](1, substream, cmd, dai);
		break;
...
//			if (_set_clock[SNDRV_PCM_STREAM_CAPTURE])
//				_set_clock[SNDRV_PCM_STREAM_CAPTURE](0, NULL, 0, NULL);
//			if (_set_clock[SNDRV_PCM_STREAM_PLAYBACK])
//				_set_clock[SNDRV_PCM_STREAM_PLAYBACK](0, NULL, 0, NULL);
		}
		break;
	default:
		ret = -EINVAL;
                                ...
                                struct asoc_simple_dai *simple_dai)
{
    ...
    if (simple_dai->slots) {
        ...
        if (ret && ret != -ENOTSUPP) {
            dev_err(dai->dev, "simple-card: set_bclk_ratio error\n");
            return ret;
        }
    }
                                        ...
                                        bool is_top_level_node)
{
    ...
    struct device_node *cpu = NULL, *plat = NULL, *codec = NULL;
    char prop[128];
    char *prefix = "";
    int ret, single_cpu;
    ...
    if (!cpu || !codec) {
        ret = -EINVAL;
        dev_err(dev, "%s: cpu or codec node missing\n", __func__);
    ...
    ret = snd_soc_of_parse_tdm_slot(cpu,
                &cpu_dai->tx_slot_mask, &cpu_dai->rx_slot_mask,
                &cpu_dai->slots, &cpu_dai->slot_width);
    if (ret < 0)
        goto dai_link_of_err;

    ret = snd_soc_of_parse_tdm_slot(codec,
                &codec_dai->tx_slot_mask, &codec_dai->rx_slot_mask,
                &codec_dai->slots, &codec_dai->slot_width);
    if (ret < 0)
        goto dai_link_of_err;
                                     ...
                                     struct seeed_card_data *priv)
{
    ...
    if (dai_link) {
        ...
        for_each_child_of_node(node, np) {
            ret = seeed_voice_card_dai_link_of(np, priv, i, false);
            if (ret < 0) {
                of_node_put(np);
                goto card_parse_end;
            }
...
static int seeed_voice_card_probe(struct platform_device *pdev)
{
	...
	/* Verifica quantos dai_links existem no device tree */
	if (np && of_get_child_by_name(np, PREFIX "dai-link"))
		num = of_get_child_count(np);
	else
		num = 1;

	/* Aloca memória para as estruturas principais */
	...
	/* Preenche os componentes padrão de cada dai_link */
	for (i = 0; i < num; i++) {
	...
	/* Preenche estrutura do snd_card */
	...
	if (np && of_device_is_available(np)) {
		...
		if (ret < 0) {
			if (ret != -EPROBE_DEFER)
				dev_err(dev, "Erro ao fazer parse do DT: %d\n", ret);
			goto err;
		}
		} else {
		...
		if (!cinfo->name || !cinfo->codec_dai.name ||
		...
		codecs->of_node    = NULL;  // usamos NULL por segurança
	...
	/* Guarda o driver data no snd_card */
...
static struct platform_driver seeed_voice_card = {
	...
	.remove = seeed_voice_card_remove, // tipo corrigido para void
};
...
MODULE_AUTHOR("Seeed Technology <www.seeedstudio.com>, Wartem, Adaptado por ChatGPT");
